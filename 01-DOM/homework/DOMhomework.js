...
created.innerHTML = created.innerHTML + ' Leandro';
...
function buildToDos(toDos) {
  // Tu código acá:
  const objToDos = toDos.map(buildToDo);
  return objToDos;
}
...
function displayToDos() {
  ...
  buildedToDoItems = buildToDos(toDoItems);
  for(let i = 0; i < buildedToDoItems.length; i++) {
    toDoContainer.appendChild(buildedToDoItems[i])
  }
}
